...
export default function SessionPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-6" style={{ backgroundColor: 'var(--color-bg-base)' }}>
      <div className="w-full max-w-sm flex flex-col gap-6">
        {/* Session name */}
        <div className="text-center">
          <p className="text-xs font-bold tracking-widest" style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-accent)' }}>
            {session.code}
          </p>
          <p className="text-sm mt-1" style={{ color: 'var(--color-text-secondary)' }}>{session.name}</p>
        </div>
        ...
        {openQuestion && answered && (
          <div
            ...
            style={{ backgroundColor: 'var(--color-bg-surface)', border: '1px solid var(--color-accent)' }}
          >
            <span className="text-3xl">✓</span>
            <p className="font-medium" style={{ color: 'var(--color-accent)' }}>Réponse envoyée !</p>
            <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
        ...
        {openQuestion && !answered && !alreadyAnswered && (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                  ...
                  {openQuestion.choices.map((choice) => (
                    <button
                      ...
                      style={selected === choice
                        ...
                        : { backgroundColor: 'var(--color-bg-elevated)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }
                      }
                    >
                      {choice}
                    </button>
              ...
              {submitError && <p className="text-sm" style={{ color: 'var(--color-error)' }}>{submitError}</p>}

              <button
                type="submit"
                disabled={submitting || (openQuestion.type === 'mcq' ? !selected : !wordInput.trim())}
